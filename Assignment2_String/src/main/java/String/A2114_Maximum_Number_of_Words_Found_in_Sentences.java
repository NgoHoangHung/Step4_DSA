 ...
 */
public class A2114_Maximum_Number_of_Words_Found_in_Sentences {
    public static void main(String[] args) {
String[] str = {"alice and bob love leetcode", "i think so too", "this is great thanks very much"};
        System.out.println(mostWordsFound2(str));
    }

    public static int mostWordsFound(String[] sentences) {
        int output = 0;
        for (String sentence : sentences) {
            String[] stringtmp = sentence.split("\\s");
            if (output < stringtmp.length)
                output = stringtmp.length;
        }
        return output;
    }
    ...
    public static int mostWordsFound1(String[] sentences) {
        int count = 0;
        for (String sentence : sentences) {
            if(count<sentence.split("\\s").length)
